...
export async function POST(req: NextRequest) {
  ...
  if (!conversationId || !userId) return NextResponse.json({ error: "Invalid input" }, { status: 400 });
  if (!(await isAdmin(conversationId, user.id))) return NextResponse.json({ error: "Admin only" }, { status: 403 });

  const [exists] = await db.select({ id: profiles.id }).from(profiles).where(eq(profiles.id, userId)).limit(1);
  if (!exists) return NextResponse.json({ error: "User not found" }, { status: 404 });

  await db.insert(conversationMembers).values({ conversationId, userId, role: "member" }).onConflictDoNothing();
  return NextResponse.json({ ok: true }, { status: 201 });
}

// DELETE /api/group-members  { conversationId, userId }
// Admin can remove any member. Any member can remove themselves (leave).
export async function DELETE(req: NextRequest) {
  ...
  if (isSelf && !(await isMember(conversationId, user.id))) {
